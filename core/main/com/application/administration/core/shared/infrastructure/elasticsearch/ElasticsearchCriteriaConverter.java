...
public final class ElasticsearchCriteriaConverter {
    private final HashMap<FilterOperator, Function<Filter, QueryBuilder>> queryTransformers = new HashMap<FilterOperator, Function<Filter, QueryBuilder>>() {{
        ...
        put(FilterOperator.GEO_DISTANCE, ElasticsearchCriteriaConverter.this::geoDistanceQueryTransformer);
    }};
    ...
    private QueryBuilder geoDistanceQueryTransformer(Filter filter) {
        double[] filterValue = formatGeoValues(filter.value().value());
        return  geoDistanceQuery(filter.field().value())
                .point(filterValue[0], filterValue[1])
                .distance(filterValue[2], DistanceUnit.KILOMETERS);
    }
    ...
    private double[] formatGeoValues(String data) {
        String[] filterValue = data.split(",");
        return Arrays.stream(filterValue).mapToDouble(Double::parseDouble).toArray();
    }
}
